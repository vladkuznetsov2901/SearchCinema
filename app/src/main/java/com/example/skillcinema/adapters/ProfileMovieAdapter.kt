...
class ProfileMovieAdapter : ListAdapter<Movie, RecyclerView.ViewHolder>(DiffUtilCallback()) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ...
            TYPE_CLEAR_ALL_BUTTON -> {
                val binding = DeleteHistoryButtonBinding.inflate(
                    LayoutInflater.from(parent.context),
                    parent,
                    false
                )
        ...
        when (holder) {
            ...
            is ClearHistoryViewHolder -> {
                with(holder.binding) {
                    ...
                    deleteHistoryBtn.visibility = if (position == itemCount - 1) View.VISIBLE else View.GONE
                    deleteHistoryText.visibility = if (position == itemCount - 1) View.VISIBLE else View.GONE
                }
            }
        }
    }
